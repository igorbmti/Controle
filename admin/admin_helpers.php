    ...
    <aside class="sidebar">
        ...
        <nav class="nav" aria-label="Navegação principal">
            ...
            <div class="nav-group">
                ...
                <?php echo adminNavItem('movimentacoes.php', 'Movimentações', $icons['reports'], $current); ?>
            </div>
        </nav>
    ...
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .brand span:first-child{ color:#fff; } .brand span:nth-child(2){ color:var(--red); } .brand .plus{ color:#fff; background:var(--red); width:18px; height:18px; border-radius:50%; font-size:15px; line-height:17px; display:inline-flex; align-items:center; justify-content:center; margin-left:4px; margin-top:1px; } .brand-collapsed{ display:none; }
            ...
            .panel{ background:linear-gradient(150deg,rgba(255,255,255,.045),transparent 40%),rgba(17,22,30,.88); border:1px solid var(--line); border-radius:var(--radius); overflow:hidden; margin-bottom:18px; box-shadow:0 18px 44px rgba(0,0,0,.18); transition:border-color .2s ease, transform .2s ease, box-shadow .2s ease; } .panel:hover{ border-color:rgba(255,255,255,.11); box-shadow:0 22px 48px rgba(0,0,0,.24); }
            .filters{ display:grid; grid-template-columns:repeat(6,minmax(140px,1fr)); gap:14px; padding:18px; align-items:end; } label{ display:grid; gap:7px; color:#f4f6fa; font-size:13px; font-weight:700; } input,select,textarea{ border:1px solid var(--line); border-radius:var(--radius); background:#10151c; color:#fff; padding:0 10px; font:inherit; } input,select{ height:40px; }
